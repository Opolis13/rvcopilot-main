...
fun ListFavoriteSitesScreen(
    ...
    println("on ListFavoriteSitesScreen")

    LaunchedEffect(currentUser.username) {
        viewModel.loadRvParks(currentUser.username)
    }

    Scaffold(
        topBar = {
            TopBar(
                title = "Favorite Campground Sites",
        ...
        LazyColumn(
            ...
            item {
                Button(
                    onClick = { viewModel.navigateToCreateCampsites() },
                    modifier = Modifier
                        .fillMaxWidth()
                        //.width(200.dp)
                        .height(60.dp)
            ...
            items(rvParks) { rvPark ->
                Row(
                    ...
                    println("Loading RV Park: ${rvPark.name}, image ID: ${rvPark.imageName}")
